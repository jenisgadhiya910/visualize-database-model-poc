...
console.log('commander =>>', commander._optionValues.name)
console.log('args =>>', process.argv)
let counter = 0;
let outputtedFile = `
const mongoose = require('mongoose');
const Schema = mongoose.Schema;
const ${commander._optionValues.name.toLowerCase()}Schema = new Schema({
`;
...
outputtedFile += `}
  ...
  module.exports = mongoose.model('${commander._optionValues.name.toTitleCase()}', ${commander._optionValues.name.toLowerCase()}Schema);
  `;
...
if (commander.directory && fs.existsSync(path.join(process.cwd(), commander.directory))) {
    fileName = path.join(process.cwd(), commander.directory, commander._optionValues.name.toLowerCase());
} else {
    fileName = './' + commander._optionValues.name.toLowerCase();
}
